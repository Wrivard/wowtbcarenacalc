...
async function main() {
  ...
  // Loaded up front, not just at the end, because gem NAMES come from here.
  // The snapshot labels a gem by its effect ("+9 Healing +3 Spell Damage and
  // +4 Intellect"), which is not a name — dropped into the gem FAQ verbatim
  // it produced "the most-used gems in this snapshot are +9 Healing +3 Spell
  // Damage and +2 Mana every 5 seconds, +4 Resilience Rating and +6 Stamina",
  // read by both the reader and, as FAQPage JSON-LD, by Google.
  ...
  const items = existsSync(itemsPath)
    ...
    : {};
  const gemName = (id, fallback) => items[String(id)]?.name ?? fallback;
  ...
  // Item metadata (name/icon/quality) for server-rendered icons. Merged into
  // the catalogue loaded above, never replacing it: data/items.json is shared
  // with the PvE lists, the season pages, the gems and the enchant reagents.
  // Writing only this build's ids used to silently drop two thirds of it, and
  // every item that lost its entry fell back to bare text with no icon.
